...
void nobody_moved(Context& context, const cucumber::messages::pickle_step& step, const std::smatch& matches) {
    context.moves[ROCK].clear();
    context.moves[PAPER].clear();
    context.moves[SCISSORS].clear();
}

void record_move(Context& context, const cucumber::messages::pickle_step& step, const std::smatch& matches) {
