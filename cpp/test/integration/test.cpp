...
TEST(TestExplanations, TestExplainableOwnership) {
...

    {
        ...
        for (TypeDB::ConceptMap& cm : res) {
            ...
            for (std::unique_ptr<TypeDB::Concept>& c : cm.concepts()) {
                std::cout << (int)c->getConceptType() << std::endl;
            }
...
TEST(Meta, NativeTraitFree) {
    {
        TypeDB::__NativePointer<char> c("EEEEP");
        std::cout << "Print " << c << std::endl;
    }
    std::cout << "It is out of scope now" << std::endl;
}
